...
#'
#' @note Logger class used internally by \pkg{cloud4R}
#'
cloud4RLogger <-  R6Class("cloud4RLogger",
  public = list(
    ...
    #' @description internal logger function for the \pkg{cloud4R} manager
    #' @param type logger message type, "INFO", "WARN", or "ERROR"
    #' @param text log message
    logger = function(type, text){
      if(self$verbose.info){
        cat(sprintf("[cloud4R][%s] %s - %s \n", type, self$getClassName(), text))
      }
    },
    ...
    #' @description initialize the \pkg{cloud4R} logger
